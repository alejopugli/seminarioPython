...
"""
def abrirArchivo(nomArch):
   try:
      file = open(nomArch, 'r')
      print('holaholahola')
   except PermissionError:
      sg.Popup('No tiene los permisos para leer el archivo de oficinas.')
   except FileNotFoundError:
      sg.Popup('No se encontro el archivo de oficinas')
   else:
      return file
   return None
"""
...
def tipoAyuda(ayuda, contador, ayudas, dic, sopa_window):
        
    if ayuda == 'TOTAL' or ayuda == 'PARCIAL':
...
config_layout = [
                   ...
                   [sg.Text('\n')],
                   [sg.Text(' '*28+'LETRA'),sg.InputCombo(values=['MAYUSCULA','MINUSCULA'],size=(15,1),key='LETRA')],
                   ...
                   [sg.Text(' '*26+'OFICINA'),sg.InputCombo(values=list(oficinas.keys()),size=(15,1),key='OFICINAS')], #values=list(oficinas.keys())                
                   [sg.Text('\n')],
...
def generar_sopa(dic,longitud, orientacion='HORIZONTAL',fuente='Comic',letra='MAYUSCULA'):
...
def dibujar_sopa(sopa_window, matriz, longitud, fuente='Comic'):
...
def main(argv):
    ...
    matriz = generar_sopa(dic,longitud,orientacion,fuente,letra)
    dibujar_sopa(sopa_window,matriz,masLarga(dic),fuente)


    
    
    ayudas = [ ]
    tipoAyuda(ayuda,contador,ayudas,dic,sopa_window)
    g = sopa_window.FindElement('_GRAPH_')
    ...
    while True:
        ...
        if(victoria):
            sg.Popup('Ganaste!',text_color='white',background_color=bg_color)
            break
        if event == 'HELP':
            descripcion = random.choice(ayudas)
            sg.Popup('Ayuda',descripcion,text_color='white',background_color=bg_color)
    sopa_window.Close()
